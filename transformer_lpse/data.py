...
class GraphDataset(object):
    ...
    def collate_fn(self):
        def collate(batch):
            ...
            # build adj_mat and adj_mat_op: (op for opposite)
            # this is useful for lpse to have adj_mat and the inverse
            adj_mat = torch.zeros((len(batch), max_len, max_len))
            adj_mat_op = torch.zeros((len(batch), max_len, max_len))
            ...
            for i, g in enumerate(batch):
                ...
                adj_mat[i, :g_len, :g_len] = utils.to_dense_adj(g.edge_index)
                # building opposite of adj_mat (ones when nodes are not neighbours)
                zero_indices = adj_mat[i, :g_len, :g_len] == 0
                non_zero_indices = adj_mat[i, :g_len, :g_len] != 0
                adj_mat_op[i, :g_len, :g_len][non_zero_indices] = 0
                adj_mat_op[i, :g_len, :g_len][zero_indices] = 1

            return padded_x, mask, pos_enc, lap_pos_enc, degree, default_collate(labels), adj_mat, adj_mat_op
        return collate
